...
export default class AnchorScroller {
  ...
  initScrollspy(links) {
    ...
    const observerOptions = {
      ...
      rootMargin: `-${offset}px 0px -60% 0px`, //* Динамически компенсируем высоту шапки
      threshold: 0,
    };
    ...
    this.observer = new IntersectionObserver((entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          ...
          //* Сбрасываем активные классы со всех ссылок меню
          links.forEach((l) =>
          ...
          if (activeLink) {
            activeLink.classList.add('active-link', 'text-emerald-400');
          }
        }
      });
    }, observerOptions);
